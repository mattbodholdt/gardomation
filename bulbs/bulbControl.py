...
def parseArgs():
    parser = argparse.ArgumentParser(
        description='Use to control Meross msl120d bulbs', allow_abbrev=True)
    ...
    parser.add_argument("--colorGroup", "-c", required=False, type=str, default=str(os.getenv('COLORGROUP', "random")),
                        help="Color group.  Default is %(default)s", choices=["psychedelic", "psyc", "primary", "stealth", "normal", "random", "off"])
...
async def main(logger):
    ...
    if str(args.deviceUUID) != None:
        ...
        bulbs = manager.find_devices(device_uuids=[
                                     args.deviceUUID], device_type=args.deviceType, online_status=OnlineStatus.ONLINE)
    else:
        disco = await manager.async_device_discovery()
        bulbs = manager.find_devices(
            device_type=args.deviceType, online_status=OnlineStatus.ONLINE)
    ...
    for dev in bulbs:
        ...
        if not dev.get_supports_rgb():
            ...
            logger.debug(
                f"Device {dev.name} is set to color (RGB) = {current_color} - luminance = {luminance}")

            logger.debug("Name: " + dev.name + " - " + dev.uuid +
            ...
            logger.info(
                f'Completed!  Setting color to {current_color} after {iterations} iterations.')
...
if __name__ == '__main__':
    ...
    loop.call_at(loop.time() + 1, loop.stop)
